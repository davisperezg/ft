import { FC, LazyExoticComponent, Suspense, lazy, useEffect, useState } from "react";
...
const DynamicComponentLoader = ({ nombreMenu }: { nombreMenu: string }) => {
  const [DynamicComponent, setDynamicComponent] = useState<LazyExoticComponent<FC> | null>(null);
    ...
    if (nombreMenu) {
      const cargarComponente = async () => {
        try {
          ...
          setDynamicComponent(lazy(() => Promise.resolve({ default: Component })));
        } catch (error) {
          console.error(error);
          setDynamicComponent(
            lazy(() =>
