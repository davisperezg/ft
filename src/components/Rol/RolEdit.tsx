...
const RolEdit = ({ data, closeEdit }: Props) => {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (values) => {
    ...
    await mutateAsync({ body: sendRol, id: data._id as string });

    const resources_updated = await mutateResources({
      resources: values.resources as string[],
      role: data._id,
    });
    console.log(resources_updated);
  ...
  return (
    <>
          ...
          <form className="overflow-y-auto flex-[1_0_calc(100%-78px)]">
            ...
            <TabModalPanel value={value} index={3}>
              <div className="flex flex-col mr-[16px] mb-[10px]">
                <div className="flex flex-row mt-2 w-full justify-end pr-5">
                  <label
                        ...
                        if (!value) {
                          memoPermisos.map((a) => {
                            ...
                            const checkboxesCategory =
                              document.querySelectorAll(`.group-${a.category}`);
                ...
                <div className="mt-2 grid grid-cols-[repeat(2,_1fr)] gap-[15px]">
                    ...
                    return (
                      <div key={i} className="border p-2 pr-2">
                        <div className="flex">
                          ...
                          >
                            <input
                              {...register(`${a.category}`, { value: false })}
                              onChange={(e) => {
                                //Obtenemos todos los inputs con la misma clase de la categoria
                                ...
                                if (e.target.checked) {
                                  ...
                                  const allCategorys = memoPermisos.map((x) => {
      ...
      <ToastError
        placeholder={isErrorEdit}
        message={errorEdit?.response.data.message}
      />
      <ToastError
        placeholder={isErrorResources}
        message={errorResources?.response.data.message}
      />
    </>
  );
};
