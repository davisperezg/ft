...
import { TableContext } from "../../../store/context/tableContext";
...
const THeadItem = ({
  ...
  return (
    <th
      ref={dropRef}
      className={`border-r border-solid font-bold h-[24px] whitespace-nowrap align-middle text-left p-0 relative hover:!bg-selected ${
        table.getState().columnSizingInfo.deltaOffset !== null
          ? "cursor-col-resize"
          : "cursor-pointer"
      }`}
      {...{
        style: {
          backgroundColor: isClicked ? "#e2e2e2" : "#1723360A",
    ...
    >
      ...
      <div
        {...{
          ...
          className:
            header.id !== "actions"
              ? `absolute right-0 top-0 w-[1px] cursor-col-resize select-none touch-none hover:bg-borders z-10 hover:z-10 ${
                  header.column.getIsResizing() ? "bg-borders z-10" : null
                }`
              : ``,
        }}
      />
...
const ComponentTable = ({
  ...
  const firstSortActive = columns.find((item) => item.sortDescFirst);
  const allAccesorKeys = columns.filter((item) => item.accessorKey);
  ...
  const [sorting, setSorting] = useState<SortingState>([
    {
      id:
        firstSortActive?.id ??
        firstSortActive?.accessorKey ??
        allAccesorKeys[1].id ??
        allAccesorKeys[1].accessorKey,
      desc: true,
    },
  ]);
  ...
  //OPTIONS HEADERS
  const widthHeader =
    refContentTHeader.current && refContentTHeader.current.clientWidth;
  const widthTable = leftTable;

  const widthGroup = refElement2.current && refElement2.current.clientWidth;

  const calcScrollHeigth =
    refElement2.current &&
    refElement2.current.scrollHeight > refElement2.current.clientHeight;

  const widthInScroll =
    Number(widthHeader) - Number(widthGroup) - (calcScrollHeigth ? 34 : 17);
      ...
      {loading ? (
        ...
        <>
          ...
          <div
            ...
            ref={refContentTBody}
            className="flex-[1_1_auto] relative overflow-auto w-full h-[260px] select-text border-t-1  border-solid border-t border-b text-default"
          >
            <table
              {...{
                style: {
          ...
          <div className="m-[6px_3px_6px_6px] float-left w-full whitespace-nowrap select-none">
            ...
            <div className="float-left bg-none h-[24px] m-[0_5px] align-middle whitespace-nowrap">
              <div
                ...
                className={`
            float-left w-[22px] h-[22px] border-0 overflow-hidden text-[14px] text-center pt-[2px] text-[#9b9c9c] flex items-center justify-center ${
              propsPagination
            ...
            `}
              ...
              <div
                ...
                className={`float-left w-[22px] h-[22px] border-0 overflow-hidden text-[14px] text-center pt-[2px] flex items-center justify-center ${
                  propsPagination
                    ? table.getState().pagination.pageIndex === 1
                      ? "text-textDisabled"
                      : "text-default cursor-pointer"
              ...
              <div className="float-left bg-none text-default h-[24px] m-[0_5px] align-middle whitespace-nowrap">
                <div
                  className={`float-left w-[22px] h-[22px] border-0 overflow-hidden text-[14px] text-center pt-[2px] text-[#9b9c9c] flex items-center justify-center ${
                    propsPagination
                      ? table.getState().pagination.pageIndex ===
                        table.getPageCount()
                        ? "text-textDisabled"
