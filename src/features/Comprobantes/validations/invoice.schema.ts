...
const schemaFormInvoiceObservacion = yup.object().shape<Shape<IValidationInvoiceObservaciones, true>>({
...
const schemaFormProductInvoiceDetail = yup.object().shape<Shape<IValidationProductInvoice, true>>({
...
const SchemaFormProductInvoiceDetail = schemaFormProductInvoiceDetail as yup.Schema<
  Infer<typeof schemaFormProductInvoiceDetail>
>;

const schemaFormInvoiceDetail = yup.object().shape<Shape<IValidationInvoiceDetails, true>>({
  id: yup.number().optional(),
...
const SchemaFormInvoiceDetail = schemaFormInvoiceDetail as yup.Schema<Infer<typeof schemaFormInvoiceDetail>>;

export const schemaFormInvoice = yup.object().shape<Shape<IFeatureInvoice, true>>({
  id: yup.number().optional(),
  ...
  cliente: yup.string().min(3, "Nombre inválido (min. 3 caracteres)").required("Ingrese nombre."),
